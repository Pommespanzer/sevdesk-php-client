...
/**
 * ObjectUsedApi Class Doc Comment
 *
 * @category Class
 * @package  isp-serverfarm\sevdesk\php-client
 * @author   Swagger Codegen team
...
class ObjectUsedApi
{
     ...
     * @param int $offset Set the index where the returned objects used start. Default is 0 (optional, default to 0)
     * @throws \isp-serverfarm\sevdesk\php-client\ApiException on non-2xx response
     * @return \isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelObjectUsed
     */
    public function getObjectUsed($limit = '100', $offset = '0')
    {
        list($response) = $this->getObjectUsedWithHttpInfo($limit, $offset);
        return $response;
    }

    /**
     * Operation getObjectUsedWithHttpInfo
     *
     * Get an overview of all objects used
     *
     * @param int $limit Limits the number of entries returned. Default is 100 (optional, default to 100)
     * @param int $offset Set the index where the returned objects used start. Default is 0 (optional, default to 0)
     * @throws \isp-serverfarm\sevdesk\php-client\ApiException on non-2xx response
     * @return array of \isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelObjectUsed, HTTP status code, HTTP response headers (array of strings)
     */
    public function getObjectUsedWithHttpInfo($limit = '100', $offset = '0')
    {
        // parse inputs
        $resourcePath = "/ObjectUsed";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelObjectUsed',
                '/ObjectUsed'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelObjectUsed', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelObjectUsed', $e->getResponseHeaders());
