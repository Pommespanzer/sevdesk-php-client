...
/**
 * DocumentIndexApi Class Doc Comment
 *
 * @category Class
 * @package  isp-serverfarm\sevdesk\php-client
 * @author   Swagger Codegen team
...
class DocumentIndexApi
{
     ...
     * @param int $offset Set the index where the returned document indexes start. Default is 0 (optional, default to 0)
     ...
     * @return \isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelDocumentIndex
     */
    public function getDocumentIndexes($limit = '100', $offset = '0', $embed = null)
    {
        list($response) = $this->getDocumentIndexesWithHttpInfo($limit, $offset, $embed);
        return $response;
    }

    /**
     * Operation getDocumentIndexesWithHttpInfo
     *
     * Get an overview of all document indexes
     *
     * @param int $limit Limits the number of entries returned. Default is 100 (optional, default to 100)
     ...
     * @param string[] $embed Get some additional information. Embed can handle multiple values, they must be separated by comma. Default &#x60;&#x60;. (optional)
     * @throws \isp-serverfarm\sevdesk\php-client\ApiException on non-2xx response
     * @return array of \isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelDocumentIndex, HTTP status code, HTTP response headers (array of strings)
     */
    public function getDocumentIndexesWithHttpInfo($limit = '100', $offset = '0', $embed = null)
    {
        // parse inputs
        $resourcePath = "/DocumentIndex";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelDocumentIndex',
                '/DocumentIndex'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelDocumentIndex', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\isp-serverfarm\sevdesk\php-client\sevdesk-model\ModelDocumentIndex', $e->getResponseHeaders());
